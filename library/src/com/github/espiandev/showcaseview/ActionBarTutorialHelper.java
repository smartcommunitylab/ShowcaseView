 ...
 *
 */
public class ActionBarTutorialHelper extends TutorialHelper {
	 ...
	 * @param mItemList
	 */
	public ActionBarTutorialHelper(Activity mActivity, TutorialProvider provider) {
	 ...
	 */
	public static void fillTutorialItemParams(TutorialItem item, int index, Activity activity, int itemId) {
		if (activity != null) {
			View v= activity.findViewById(itemId);
				if (v != null) {
					item.width = v.getWidth();
					item.position = new int[2];
					v.getLocationOnScreen(item.position);
			}
		}
	}
