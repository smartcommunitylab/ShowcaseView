...
/**
 * Tutorial helper class that uses a list view as the list of reference points
 * for the tutorial steps. To manage tutorial it is necessary to provide the
 * parent activity where the tutorial is shown and the {@link TutorialProvider}
 * instance to provide the information about the tutorial items and views, as
...
public class ListViewTutorialHelper extends TutorialHelper {
	...
	/**
	 * Fill in the parameters for the tutorial items using the element at
	 * 'index' in the view to calculate the position and width. If needed, the
	 * required element will be scrolled to.
	 * 
	 * @param item
	 *            {@link TutorialItem} instance to populate
	 * @param index
	 *            position of the element in the list
	 * @param listView
	 *            {@link ListView} holding the steps of the tutorial
	 * @param itemId
	 *            layout ID of the view to search for
	 */
		...
		if (listView != null) {
			...
			if (index <= firstVisible) {
				...
				if (v != null && v.findViewById(itemId) != null)
					...
					&& !(index == listView.getAdapter().getCount() - 1)) {
	...
	public static void fillTutorialItemParamsWithCorrection(TutorialItem item, int index, ListView listView, int itemId, int xCorrection, int yCorrection) {
		if (listView != null) {
			...
			if (v != null) {
				...
				if (logo != null) {
					...
					item.position = new int[2];
					logo.getLocationOnScreen(item.position);
					item.position[0] += xCorrection;
					item.position[1] += shift +yCorrection; // correction
				}
			}
		}
	}
	...
	private void displayShowcaseView(TutorialItem item, boolean isLast) {
		BaseTutorialActivity.newIstance(mActivity, item.position, item.width, Color.WHITE, null,
